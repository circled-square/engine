...
namespace gal {
    ...
    void renderer::draw(const vertex_array& vao, const shader_program& shader, size_t ibo_start, size_t ibo_count, unsigned ibo_index) {
        ...
        glDrawElements(GL_TRIANGLES, (GLsizei)ibo_count * 3, ibo_elem_typeid, reinterpret_cast<const void*>(ibo_elem_size * 3 * ibo_start)); //draw triangles({1}), read {2} elements of type {3} starting from the {4}th byte from the bound GL_ELEMENT_ARRAY_BUFFER
    }
